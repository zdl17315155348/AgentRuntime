...
async def _run(mode: ExecutionMode, repo: Path, require_real: bool) -> int:
    ...
    print(json.dumps({"run_id": config.run_id, "mode": mode.value, "status": summary.get("status"), "summary": str(out_dir / "summary.json")}, ensure_ascii=False))
...
def main() -> int:
    ...
    return asyncio.run(_run(ExecutionMode.DIRECT, Path(args.source_repo).resolve(), args.require_real))

...
async def _run(
    ...
    if require_real:
        ...
        if llm_backend == "mock":
            print("[FAIL] agentd is using mock LLM backend; set LLM_BACKEND=deepseek and RUNTIME_CONFIG to the repo runtime config")
            return 1
    ...
    manifest_path = _write_manifest("runtime", config.run_id, str(summary.get("status")), out_dir / "summary.json", config, repo_info, evidence_dir, started_at, finished_at)
    print(json.dumps({"run_id": config.run_id, "mode": "runtime", "status": summary.get("status"), "summary": str(out_dir / "summary.json"), "manifest": str(manifest_path)}, ensure_ascii=False))
...
def main() -> int:
    ...
    parser.add_argument("--evidence-dir", default=str(ROOT / "final-evidence/runtime-e2e"))

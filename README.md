...
- [x] openEuler Codex 写入沙箱依赖：Docker 镜像显式安装 `bubblewrap`，避免 Direct/Runtime 的 `workspace-write` Codex 执行返回 `bubblewrap is unavailable` 且不产出 patch；依据：openEuler 容器内 `command -v bwrap` 与 `bwrap --ro-bind / / true` 探针。
- [x] Planner 截断 JSON 重试：共享 Planner Pipeline 对 DeepSeek 返回的截断/非法 JSON 最多重试 4 次，并在重试 prompt 中附带解析错误，避免一次 `Unterminated string` 直接终止 E2E；依据：`python3 -m pytest testing/unittest/applications/test_shared_planner_pipeline.py -q`。
- [x] Coder no-op 完成路径：Direct coder 解析 Codex JSON 结构化结果，`completed=true` 且 `remaining_issues=[]` 时允许无 patch 子任务标记完成，避免前序 patch 已覆盖目标时最后一个子任务误报 `coder produced no patch`；依据：`python3 -m pytest testing/unittest/applications/test_coder_base_commit.py testing/unittest/applications/test_incident_execution_provider.py -q`。
- [x] Repair Review 上下文传递：`repair_node` 将 `review_summary` 传给修复 Agent，并更新 repair prompt 覆盖 reviewer 拒绝场景，避免 reviewer blocking issue 无法驱动修复而返回 no patch；依据：`python3 -m pytest testing/unittest/applications/test_repair_failure.py -q`。

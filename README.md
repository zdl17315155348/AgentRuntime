...
统一资源管理：Runtime 统一管理 CPU、Memory、LLM Concurrency、Token、Tool、KV Cache、Network。资源模型包含 `ResourceClass`、`ResourceQuota`、`ResourceRequest`、`ResourceUsage`、`ResourceLease`、`ResourceReclaimer`；调度前检查资源，执行前申请 lease，执行中监控资源，执行后释放资源，超限时进入 retry / fallback / kill 流程。
...
Benchmark：根目录 `BENCHMARK.md` 记录调度、上下文、容错、通信 4 组对比实验的过程、复现方法、指标定义和结果；通过 `bash scripts/benchmark_docker_openeuler.sh` 在 openEuler Docker 中生成。
...
资源控制：CgroupManager 负责 create / attach / update / read_stats / kill / cleanup，控制 cpu.max、cpu.weight、memory.high、memory.max、pids.max，并读取 cpu.stat、memory.events、cgroup.events。
...
perf/test_benchmark.py:Benchmark 实验（FIFO 串行 vs 并发 resource-aware、无复用 vs context/prefix reuse、失败级联 vs fail-open/fallback、HTTP polling vs UDS/mailbox），输出根目录 `BENCHMARK.md`。
...
resource/test_resources.py:资源租约、LLM 并发、cgroup manager 单元测试。
...
Benchmark 入口为 `scripts/benchmark_docker_openeuler.sh`，在 openEuler Docker 容器中执行 `testing/perf/test_benchmark.py` 并复制生成根目录 `BENCHMARK.md`。
